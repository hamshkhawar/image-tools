...
def main(inpDir:Path,
        ...
        segDir = SMP_training_inference(inpDir=corrDir, filePattern=filePattern, outDir=outDir,dryrun=True)
        logger.info("Step9: Finished Running SMP_training_inference plugin")
        logger.info("Step10: FtlLabel plugin is running")
        segDir = FtlLabel(inpDir=outpath, outDir=outDir, dryrun=True)
        logger.info("Step10: Finished Running FtlLabel plugin")
        logger.info("Step12: Nyxus plugin is Running")
        Nyxus_exe(inpDir=corrDir, segDir=segDir, filePattern=filePattern, outDir=outDir, dryrun=True)
