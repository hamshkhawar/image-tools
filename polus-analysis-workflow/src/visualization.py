...
parser = argparse.ArgumentParser(prog='main', description='Visualization Workflow')    
#     # Input arguments
...
def main(data:str,
        ...
        for plate in plates:    
            ...
            outpath = stichingvector(inpDir=outpath, outDir=outDir)
            ...
            logger.info(f"Finished running stichingvector: {(time.time() - rectime)/60}")
            assemtime= time.time()
            ...
            outpath = precompute_slide(inpDir=outpath, plate=plate, pyramidType='Zarr', imageType='image', outDir=outDir, dryrun=False)
